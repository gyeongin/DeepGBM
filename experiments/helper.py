import argparse, os, logging, random, time, datetime
...
def EvalTestset(test_x, test_y, model, test_batch_size, test_x_opt=None):
    ...
    with torch.no_grad():
        for jdx in range(test_num_batch):
            ...
            batch_test_x = test_x[tst_st:tst_ed]
            if isinstance(batch_test_x, scipy.sparse.csr_matrix):
                batch_test_x = batch_test_x.toarray()
            inputs = torch.from_numpy(batch_test_x.astype(np.float32)).to(device)
...
def TrainWithLog(args, plot_title, train_x, train_y,
                 train_y_opt, valid_x, valid_y, test_x, test_y, model, opt,
                 epoch, batch_size, n_output, key="",
                 train_x_opt=None, valid_x_opt=None, test_x_opt=None):
    ...
    for epoch in range(epoch):
        ...
        for local_iter in range(train_num_batch):
            ...
            if isinstance(batch_trn_x, scipy.sparse.csr_matrix):
                batch_trn_x = batch_trn_x.toarray()
            ...
            if global_iter % args.log_freq == 0:
                print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " " + key + "Epoch-{:0>3d} {:>5d} Batches, Step {:>6d}, Training Loss: {:>9.6f} (AllAvg {:>9.6f})"
                            .format(epoch, local_iter + 1, global_iter, Loss_trn_log/(trn_ed-log_st), Loss_trn_epoch/trn_ed), flush=True)
            ...
            if global_iter % args.test_freq == 0 or local_iter == train_num_batch - 1:
                ...
                valid_loss, pred_y = EvalTestset(valid_x, valid_y, model, args.test_batch_size, valid_x_opt)
                ...
                if args.task == 'binary':
                    metrics = eval_metrics(args.task, valid_y, pred_y)
                    _, valid_auc = metrics
                    ...
                    print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " " + key + "Evaluate Result:\nEpoch-{:0>3d} {:>5d} Batches, Step {:>6d}, Validation Loss: {:>9.6f}, Validation AUC: {:8.6f}, Used Time: {:>5.1f}m, Remaining Time: {:5.1f}m"
                            .format(epoch, local_iter + 1, global_iter, valid_loss, valid_auc, total_time/60.0, remaining_time/60.0), flush=True)
                    ...
                    if valid_auc > max_auc:
                        ...
                        _, test_pred_y = EvalTestset(test_x, test_y, model, args.test_batch_size, test_x_opt)
                        _, test_auc = eval_metrics(args.task, test_y, test_pred_y)
                        print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " Epoch-{:0>3d} Got best Validation AUC: {:8.6f}, Test AUC: {:8.6f}".format(epoch, valid_auc, test_auc), flush=True)
                else:
                    print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " " + key + "Evaluate Result:\nEpoch-{:0>3d} {:>5d} Batches, Step {:>6d}, Validation Loss: {:>9.6f}, Used Time: {:>5.1f}m, Remaining Time: {:5.1f}m"
                            .format(epoch, local_iter + 1, global_iter, valid_loss, total_time/60.0, remaining_time/60.0), flush=True)
    ...
    if 'emb' not in key:
        ckpt = torch.load(args.ckpt_path + '/best.ckpt')
        print("Load the best checkpoint from epoch %d" % ckpt['epoch'])
        model.load_state_dict(ckpt['model_state_dict'])
        opt.load_state_dict(ckpt['optimizer_state_dict'])

    valid_loss, valid_pred_y = EvalTestset(valid_x, valid_y, model, args.test_batch_size, valid_x_opt)
    test_loss, test_pred_y = EvalTestset(test_x, test_y, model, args.test_batch_size, test_x_opt)
    _, valid_auc = eval_metrics(args.task, valid_y, valid_pred_y)
    _, test_auc = eval_metrics(args.task, test_y, test_pred_y)
    print("Final result: Validation Loss: {:>9.6f}, Validation AUC: {:8.6f}, Test Loss: {:>9.6f}, Test AUC: {:8.6f}".format(valid_loss, valid_auc, test_loss, test_auc))

    return min_loss

def GetEmbPred(model, fun, X, test_batch_size, emb_size):
    model.eval()
    tst_len = X.shape[0]
    test_num_batch = math.ceil(tst_len / test_batch_size)
    y_preds = np.zeros((tst_len, emb_size), dtype=np.float32)
    with torch.no_grad():
        for jdx in range(test_num_batch):
            tst_st = jdx * test_batch_size
